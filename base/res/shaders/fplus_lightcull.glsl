...
float getDepth (in uint pixel_x, in uint pixel_y)
{
	...
	return texture (depthTex, texCoords);
}
...
void checkAndAppendLight (in uint lightIndex, in int buffertype)
{
	...
	if (buffertype == 1)
		...
		viewSpaceLightPos = u_View * vpl [lightIndex];

	if ((viewSpaceLightPos.z >= (depthMin-epsilon)) && (viewSpaceLightPos.z >= (depthMin-epsilon)))
	{
		// TODO: Evaluate equations and add light to lightList.
		;
	}
}

vec4 projectionToViewSpace (in uint x, in uint y, in float depth)
{
	;
}
...
void main ()
{
	if ((gl_LocalInvocationID.x == 0) && (gl_LocalInvocationID.y == 0))
	{
		...
		for (int i = 0; i < 4; ++ i)
			frustum [i] = makePlane (vec4(0.0), v[i], v[(i+1)&3]);
